...
//echo Page::getByID($page->getCollectionID(), 1)->getVersionObject()->getVersionAuthorUserName(); //get username of original author
...
//echo Page::getByID($page->getCollectionParentID())->getCollectionName(); //get parent name
...
$page = $parentPage->add(
    ...
    array(
    	...
    	//'cDateCreated' => date('Y-m-d H:i:s'), //updates the posting date of the page to right now
    	//'pkgID ' => 1,
    	//'uID ' => 1,
    ),
    $pageTemplate
...
if (!is_object($attr)) {
    ...
    //add options to an 'option list' attribute
